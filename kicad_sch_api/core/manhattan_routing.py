...
from .component_bounds import BoundingBox, get_component_bounding_box, check_path_collision
...
class GridPoint:
    ...
    def from_world_point(cls, point: Point, grid_size: float = 1.27) -> 'GridPoint':
        ...
        return cls(
            x=round(point.x / grid_size),
            y=round(point.y / grid_size)
        )
...
class ManhattanRouter:
    ...
    def _find_path_astar(
        ...
        start_node = PathNode(
            position=start,
            g_cost=0,
            h_cost=self._heuristic_cost(start, goal, strategy)
        )
    ...
    def _heuristic_cost(self, point: GridPoint, goal: GridPoint, strategy: RoutingStrategy) -> float:
