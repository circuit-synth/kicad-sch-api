...
class TestPinPositioning:
    ...
    def test_get_component_pin_position_basic(self, simple_schematic):
        ...
        # For 0° rotation, Pin 1 should be left of Pin 2
        assert pin1_pos.x < pin2_pos.x, "Pin 1 should be left of Pin 2 for 0° rotation"
        assert abs(pin1_pos.y - pin2_pos.y) < 0.1, "Pins should be at same Y level for 0° rotation"
    ...
    def test_get_component_pin_position_with_rotation(self):
        ...
        # For 90° rotation, Pin 1 should be above Pin 2
        assert pin1_pos.y < pin2_pos.y, "Pin 1 should be above Pin 2 for 90° rotation"
        assert abs(pin1_pos.x - pin2_pos.x) < 0.1, "Pins should be at same X level for 90° rotation"
...
class TestWireToPin:
    ...
    def test_connect_pins_with_wire(self, wire_test_schematic):
        ...
        connects_correctly = (
            (abs(wire_start.x - r1_pin2_pos.x) < 0.1 and abs(wire_start.y - r1_pin2_pos.y) < 0.1 and
             abs(wire_end.x - r2_pin1_pos.x) < 0.1 and abs(wire_end.y - r2_pin1_pos.y) < 0.1) or
            (abs(wire_start.x - r2_pin1_pos.x) < 0.1 and abs(wire_start.y - r2_pin1_pos.y) < 0.1 and
             abs(wire_end.x - r1_pin2_pos.x) < 0.1 and abs(wire_end.y - r1_pin2_pos.y) < 0.1)
        )
    ...
    def test_add_wire_to_pin(self, wire_test_schematic):
        ...
        start_matches = (abs(wire_start.x - start_point.x) < 0.1 and 
                        abs(wire_start.y - start_point.y) < 0.1)
        end_matches = (abs(wire_end.x - pin_pos.x) < 0.1 and 
                      abs(wire_end.y - pin_pos.y) < 0.1)
        
        # Could be either direction
        connects_correctly = (start_matches and end_matches) or (
            abs(wire_start.x - pin_pos.x) < 0.1 and abs(wire_start.y - pin_pos.y) < 0.1 and
            abs(wire_end.x - start_point.x) < 0.1 and abs(wire_end.y - start_point.y) < 0.1
        )
...
class TestPinOrientationAndTransformations:
    ...
    def test_pin_orientation_affects_label_direction(self):
        ...
        for i, case in enumerate(test_cases):
            ...
            # Verify label exists
            labels = [label for label in sch._data.get("labels", []) if label.get("text") == f"NET{i+1}"]
...
class TestConnectionWorkflows:
    ...
    def test_voltage_divider_with_labels(self):
        ...
        for net_name in ["VIN", "VOUT", "GND"]:
            labels = [label for label in sch._data.get("labels", []) if label.get("text") == net_name]
