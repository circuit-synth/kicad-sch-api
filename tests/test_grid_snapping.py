...
class TestGridSnapping:
    ...
    def test_snap_to_grid_basic(self):
        ...
        for input_pos, expected_pos in test_cases:
            ...
            assert abs(result[0] - expected_pos[0]) < 0.001, f"X snap failed for {input_pos}: got {result[0]}, expected {expected_pos[0]}"
            assert abs(result[1] - expected_pos[1]) < 0.001, f"Y snap failed for {input_pos}: got {result[1]}, expected {expected_pos[1]}"
            
            # Verify result is actually on grid
            x_grid_units = result[0] / grid_size
            y_grid_units = result[1] / grid_size
            
            assert abs(x_grid_units - round(x_grid_units)) < 0.001, f"X not on grid: {result[0]} = {x_grid_units} grid units"
            assert abs(y_grid_units - round(y_grid_units)) < 0.001, f"Y not on grid: {result[1]} = {y_grid_units} grid units"
    ...
    def test_component_position_snapping(self):
        ...
        for i, pos in enumerate(test_positions):
            ...
            assert abs(x_grid_units - round(x_grid_units)) < 0.001, f"{ref} X not on grid: {actual_pos.x} = {x_grid_units} grid units"
            assert abs(y_grid_units - round(y_grid_units)) < 0.001, f"{ref} Y not on grid: {actual_pos.y} = {y_grid_units} grid units"
    ...
    def test_pin_positions_on_grid(self):
        ...
        for pin_name, pin_pos in [("Pin 1", pin1_pos), ("Pin 2", pin2_pos)]:
            ...
            assert abs(x_grid_units - round(x_grid_units)) < 0.001, f"{pin_name} X not on grid: {pin_pos.x} = {x_grid_units} grid units"
            assert abs(y_grid_units - round(y_grid_units)) < 0.001, f"{pin_name} Y not on grid: {pin_pos.y} = {y_grid_units} grid units"
    ...
    def test_component_rotation_preserves_grid(self):
        ...
        for rotation in [0, 90, 180, 270]:
            ...
            for pin_name, pin_pos in [("Pin 1", pin1_pos), ("Pin 2", pin2_pos)]:
                ...
                assert abs(x_grid_units - round(x_grid_units)) < 0.001, f"{pin_name} X not on grid at {rotation}°: {pin_pos.x} = {x_grid_units} grid units"
                assert abs(y_grid_units - round(y_grid_units)) < 0.001, f"{pin_name} Y not on grid at {rotation}°: {pin_pos.y} = {y_grid_units} grid units"
    ...
    def test_multiple_components_grid_alignment(self):
        ...
        for comp in components:
            ...
            assert abs(x_grid_units - round(x_grid_units)) < 0.001, f"{comp.reference} X not on grid"
            assert abs(y_grid_units - round(y_grid_units)) < 0.001, f"{comp.reference} Y not on grid"
        ...
        for i in range(len(components) - 1):
            ...
            assert abs(x_grid_diff - round(x_grid_diff)) < 0.001, f"Components not grid-aligned in X"
            assert abs(y_grid_diff - round(y_grid_diff)) < 0.001, f"Components not grid-aligned in Y"
...
class TestGridConstants:
    ...
    def test_kicad_grid_size(self):
        ...
        assert abs(grid_in_mils - 50.0) < 0.1, f"Grid should be 50 mils, got {grid_in_mils:.2f} mils"
    ...
    def test_common_kicad_spacings(self):
        ...
        for spacing in common_spacings:
            ...
            assert abs(grid_units - round(grid_units)) < 0.001, f"Spacing {spacing}mm = {grid_units} grid units (should be whole number)"
