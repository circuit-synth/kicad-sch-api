...
class TestRoundTrip:
    ...
    def test_round_trip_rotated_resistor(self, temp_dir):
        ...
        original_path = Path('tests/reference_kicad_projects/rotated_resistor_0deg/rotated_resistor_0deg.kicad_sch')
        ...
        assert len(original_components) == len(reloaded_components), \
            f"Component count mismatch: {len(original_components)} → {len(reloaded_components)}"

        assert len(original_wires) == len(reloaded_wires), \
            f"Wire count mismatch: {len(original_wires)} → {len(reloaded_wires)}"

        assert len(original_labels) == len(reloaded_labels), \
        ...
        for orig_comp, regen_comp in zip(original_components, reloaded_components):
            assert orig_comp.reference == regen_comp.reference, \
                f"Reference mismatch: {orig_comp.reference} → {regen_comp.reference}"

            assert orig_comp.value == regen_comp.value, \
                f"Value mismatch: {orig_comp.value} → {regen_comp.value}"

            assert orig_comp.lib_id == regen_comp.lib_id, \
    ...
    def test_round_trip_with_utility_function(self, temp_dir):
        ...
        create_func = next(
            (obj for name, obj in exec_globals.items()
             if callable(obj) and name.startswith('create_')),
            None
        )
    ...
    def test_round_trip_preserves_wire_connectivity(self, temp_dir):
        ...
        for wire in wires:
            assert not (wire.start.x == 0 and wire.start.y == 0 and
                       wire.end.x == 0 and wire.end.y == 0), \
    ...
    def test_round_trip_generated_code_is_idempotent(self, temp_dir):
        ...
        create_func_1 = next(
            (obj for name, obj in exec_globals_1.items()
             if callable(obj) and name.startswith('create_')),
            None
        )

        create_func_2 = next(
            (obj for name, obj in exec_globals_2.items()
             if callable(obj) and name.startswith('create_')),
            None
        )
