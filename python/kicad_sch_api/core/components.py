...
from .types import Point, SchematicSymbol, SchematicPin
from ..library.cache import get_symbol_cache, SymbolDefinition
from ..utils.validation import ValidationError, ValidationIssue, SchematicValidator
...
class Component:
    ...
    def update_from_library(self) -> bool:
        ...
        if not self.reference.startswith(symbol_def.reference_prefix):
            logger.warning(f"Reference {self.reference} doesn't match expected prefix {symbol_def.reference_prefix}")
    ...
    def __repr__(self) -> str:
        ...
        return (f"Component(ref='{self.reference}', lib_id='{self.lib_id}', "
...
class ComponentCollection:
    ...
    def filter_by_type(self, component_type: str) -> List[Component]:
        ...
        return [c for c in self._components 
                if c.symbol_name.upper().startswith(component_type.upper())]
    ...
    def near_point(self, point: Union[Point, Tuple[float, float]], radius: float) -> List[Component]:
